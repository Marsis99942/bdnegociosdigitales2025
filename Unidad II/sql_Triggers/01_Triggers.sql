...
go
create or alter trigger tg_pedidos_actualizar_stock
on pedidos
after insert, delete
as
begin
    ...
    -- 🔹 manejo de inserción (disminuir stock)
    ...
    begin
        ...
        if exists (
            select 1
            from inserted i
            join productos p on i.fab = p.id_fab and i.producto = p.id_producto
            where i.cantidad > p.stock
        )
        begin
            raiserror('No hay suficiente stock para uno o más productos en el pedido', 16, 1);
        ...
        from productos p
        inner join inserted i on i.fab = p.id_fab and i.producto = p.id_producto;
    end

    -- 🔹 manejo de eliminación (aumentar stock)
    if exists (select 1 from deleted)
    begin
        update p
        set p.stock = p.stock + d.cantidad
        from productos p
        inner join deleted d on d.fab = p.id_fab and d.producto = p.id_producto;
    end
end;
